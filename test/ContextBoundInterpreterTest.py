...
class ContextBoundInterpreterWithKeyValueDereferencerTest(TestCase):
    ...
    def test_sum_of_visit(self):
        ...
        self.assertEqual("+", self.visitor_fixed.visit(tree).operation)
        self.assertEqual("A1",self.visitor_fixed.visit(tree).children[0].value)
    ...
    def test_sum_of_visit_with_ref_and_changes(self):
        self.helper.writeNumber("A1", 12)
        self.dereferencer.write("A2", parser.parse("A1*2"))
        self.assertEqual(37, self.visitor_volatile.visit(parser.parse("1 + A2 + A1")).value)
        self.helper.writeNumber("A1", 10)
        self.assertEqual(31, self.visitor_volatile.visit(parser.parse("1 + A2 + A1")).value)

    def test_double_indirection(self):
        self.helper.writeNumber("A1", 12)
        self.dereferencer.write("A2", parser.parse("A1"))
        self.dereferencer.write("A3", parser.parse("A2"))
        self.assertEqual(12, self.visitor_volatile.visit(parser.parse("A3")).value)
        self.helper.writeNumber("A1", 10)
        self.assertEqual(10, self.visitor_volatile.visit(parser.parse("A3")).value)

    def test_cycle(self):
        self.dereferencer.write("A1", parser.parse("A1"))
        self.assertEquals("A1", self.visitor_fixed.visit(parser.parse("A1")).value)

        with self.assertRaises(RecursionError):
            self.visitor_volatile.visit(parser.parse("A1"))
...
class ContextBoundInterpreterWithFormulaAndValueCacheDereferencerTest(TestCase):
    ...
    def test_cycle(self):
        self.dereferencer.write("A1", parser.parse("A1"), None)
        self.assertEqual("A1", self.visitor_fixed.visit(parser.parse("A1")).value)
        self.assertEqual(2, self.visitor_volatile.visit(parser.parse("A1+2")).value)
